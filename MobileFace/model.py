"""MobileFaceDetection Demo script based on Yolo_V3."""
...
from MobileFace.mobileface_net import mobilefacedetnet_v1
...
def get_faces_mobile(img_path, img_mobile):
    mobile_model = MobileFaceDetection('MobileFace/mobilefacedet_v1_gluoncv.params', "0,")
    faces_mobile = mobile_model.mobileface_detector(image_dir=img_path, image_mat=img_mobile)
    return faces_mobile


def draw_box_mobile(img_path, img_mobile):
    faces = get_faces_mobile(img_path, img_mobile)
    if not faces or len(faces) < 1:
        print("No faces detection")
    else:
        for i, result in enumerate(faces):
            xmin, ymin, xmax, ymax, score, classname = result
            cv2.rectangle(img_mobile, (xmin, ymin), (xmax, ymax), (0, 0, 255), 2)
