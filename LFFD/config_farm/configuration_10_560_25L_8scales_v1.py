...
# init logging
param_log_mode = 'w'
param_log_file_path = '../log/%s_%s.log' % \
                      (os.path.basename(__file__)[:-3], datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S'))

# data setting
# pick file path for train set
param_trainset_pickle_file_path = os.path.join(os.path.dirname(__file__),
                                               '../data_provider_farm/data_folder/train_data_gt_9.pkl')
...
param_save_prefix = '../saved_model/' + os.path.basename(__file__)[:-3] + '_' + datetime.datetime.now().strftime(
    '%Y-%m-%d-%H-%M-%S') + \
...
def run():
    ...
    from LFFD.data_iterator_farm.multithread_dataiter_for_cross_entropy_v1 \
