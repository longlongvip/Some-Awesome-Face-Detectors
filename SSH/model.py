...
from SSH.rcnn.core.module import MutableModule
from mxnet import ndarray as nd
from SSH.rcnn.config import config, default
from SSH.rcnn.processing.bbox_transform import nonlinear_pred, clip_boxes
from SSH.rcnn.processing.generate_anchor import generate_anchors_fpn, anchors_plane
from SSH.rcnn.processing.nms import gpu_nms_wrapper
...
class SSHDetector:
    def __init__(self, prefix, epoch, ctx_id=0, test_mode=False):
        ...
        # self._anchors_fpn =
        # dict(zip(self.fpn_keys, generate_anchors_fpn
        # (base_size=fpn_base_size, scales=self._scales, ratios=self._ratios)))
    ...
    def detect(self, img, threshold=0.05, scales=[1.0]):
        ...
        for im_scale in scales:
            ...
            for s in self._feat_stride_fpn:
                ...
                # anchors.shape, bbox_deltas.shape, scores.shape, file=sys.stderr)
...
def get_faces_ssh(img_path, img_ssh, pyramid=True):
    ssh_model = SSHDetector(prefix=default.e2e_prefix, epoch=0, ctx_id=0, test_mode=True)
    # prefix, epoch, ctx_id=0, test_mode=False
    if not pyramid:
        target_size = 1200
        max_size = 1600
        im_shape = img_ssh.shape
    ...
    faces_ssh = ssh_model.detect(img_ssh, threshold=0.05, scales=scales)
    return faces_ssh


def draw_box_ssh(img_path, img_ssh):
    faces = get_faces_ssh(img_path, img_ssh)
    if not faces or len(faces) < 1:
        print("No faces detection")
    else:
        for i in range(faces.shape[0]):
            face = faces[i]
            cv2.rectangle(img_ssh, (face[0], face[1]), (face[2], face[3]), (0, 0, 255), 2)
